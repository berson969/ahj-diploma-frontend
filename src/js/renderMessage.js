...
export default class RenderMessage {
  ...
  displayImageContent (message) {
    ...
    imageDisplay.classList.add('preview-image', "lazyload", 'mr-auto', "p-2", 'col-6','img-fluid');
  ...
  displayFileContent (message) {
    ...
    fileDisplay.classList.add('preview-file','d-flex', 'justify-content-center', "mb-2", 'col-4')
    fileDisplay.download = message.textBody;
    fileDisplay.href = message.blobContent;
    fileDisplay.innerHTML =
      `<i class="bi bi-file-earmark-code-fill" style="font-size: 100px; color: cornflowerblue;"></i>`;
    return fileDisplay;
  }

  makeMapLayer (message)  {
    if (typeof message.textBody === 'string') {
      const coordinates = message.textBody.split(":")
  ...
  renderDownloadEl (message) {
    ...
    if (message.from === this.userId) {
      this.downloadEl.classList.add('download-el', 'd-flex', 'justify-content-end');
    } else {
   ...
   renderChat (message) {
    ...
    this.messageEl.classList.add('row', "d-flex", 'align-items-end', 'm-md-3', 'mb-4')
